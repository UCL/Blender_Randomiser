...
if [ -z "${blend_file}" ] || [ -z "${seed_num}" ] || [ -z "${json_input}" ] || [ -z "${json_output}" ]; then
    correct_input=0
else
    correct_input=1
fi
...
if ( [[ ${correct_input} -eq 0 ]] ) ; then
  ...
  echo ""
  echo "${script_name} -blend /home/username/colon.blend -json_in /home/username/input.json -json_out  /home/username/output.json -json /home/username/synb0/dwi.json -seed 32"
  echo ""
  exit
fi
