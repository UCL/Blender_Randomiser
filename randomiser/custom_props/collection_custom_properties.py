...
def compute_sockets_sets(self):
    ...
    for sck in self.candidate_sockets:
        ...
        if sck.name in self.candidate_sockets:
            list_sckt_names_in_UIlist.append("UI_list" + "_" + sck.name)
        # if socket comes from an independent node
        else:
            list_sckt_names_in_UIlist.append("_" + sck.name)

    self.set_sckt_names_in_UIlist = set(list_sckt_names_in_UIlist)
...
def get_update_collection(self):
    ...
    of the class ColCustomProperties
...
def set_update_collection(self, value):
    ...
    if value:
        ...
        for sckt_name in self.set_of_sckt_names_in_one_only:
            ...
            # - if the socket exists only in the node graph: add to collection
            # with initial values
            if sckt_name in self.set_sckt_names_in_UIlist:
...
# ColCustomProperties
# ---------------------
class ColCustomProperties(bpy.types.PropertyGroup):
    ...
    update_custom_collection: bpy.props.BoolProperty(  # type: ignore
        default=False,
...
# ------------------------------------------
def register():
    bpy.utils.register_class(ColCustomProperties)


def unregister():
    bpy.utils.unregister_class(ColCustomProperties)
