...
class PropertiesAddArrayObjects2Cursor(bpy.types.PropertyGroup):
    n_objects_array_prop = bpy.props.IntProperty(
        name="Number of objects in array",
        default=10,
        description="""
            "Number of objects to generate
            between current active object and cursor"
            """,
        soft_min=0,
        soft_max=10,
    )
    n_objects_array: n_objects_array_prop  # type: ignore


# ---------------
# Operator
class AddArrayObjects2Cursor(
    bpy.types.Operator
):  # ---what other classes can I instantiate?
    ## Class properties
    ...
    def execute(self, context):
        ...
        for i in range(scene.array_objects2cursor_props.n_objects_array):
            object_copy = (
                object_active.copy()
            )  # copy to the same data block (linked copy)
            scene.collection.objects.link(object_copy)

            factor = i / scene.array_objects2cursor_props.n_objects_array
            object_copy.location = (object_active.location * factor) + (
                cursor_loc * (1.0 - factor)
            )
...
class PanelAddArrayObjects2Cursor(bpy.types.Panel):
    ...
    # panel title/label displayed to the user
    bl_label = "Add array of objects to cursor"
    ...
    def draw(self, context):
        ...
        left_col.label(
            text="Number of objects in array"
        )  # TODO: can I get this from the property?
        right_col.prop(
            context.scene.array_objects2cursor_props,
            "n_objects_array",
            icon_only=True,
        )
...
def register():
    ...
    for cls in list_classes_to_register:
        ...
        if cls == PropertiesAddArrayObjects2Cursor:
            bpy.types.Scene.array_objects2cursor_props = (
                bpy.props.PointerProperty(
                    type=PropertiesAddArrayObjects2Cursor
                )
            )
